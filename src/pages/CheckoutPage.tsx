            ...
            {showNewAddr ? (
              <div className="flex gap-2 mt-2">
                <input type="text" value={newAddress} onChange={(e) => setNewAddress(e.target.value)}
                  className="flex-1 h-9 px-3 rounded-lg bg-secondary text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30"
                  placeholder="Enter new address" />
                <button onClick={() => { toast.success("Address saved"); setShowNewAddr(false); }}
                  className="px-3 h-9 rounded-lg bg-primary text-primary-foreground text-sm font-medium">Save</button>
              </div>
            ) : (
